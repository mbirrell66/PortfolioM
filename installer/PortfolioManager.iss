...
; Version comes from the build script, which reads it from
; portfolio_manager\_version.py:
;     ISCC.exe /DAppVersion=1.3.0 installer\PortfolioManager.iss
; The fallback below only applies when compiling from the Inno Setup IDE,
; and must be kept in step with _version.py.
#ifndef AppVersion
  #define AppVersion   "1.3.0"
#endif

#define AppPublisher   "Martin Birrell"
#define AppURL         "https://github.com/mbirrell66/PortfolioM"
...
[UninstallDelete]
; The user's portfolio database lives in {app}\data and is deliberately NOT
; removed here.
;
; This is not only about uninstalling. InitializeSetup below runs the previous
; version's uninstaller silently when upgrading, so deleting {app}\data on
; uninstall would destroy the user's portfolio, income, expense and tax records
; every time they installed a new release.
;
; Uninstalling therefore leaves {app}\data behind for the user to remove by
; hand. Do not re-enable the line below without first making the upgrade path
; skip it.
